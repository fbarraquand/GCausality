...
for (ksite in 1:nsites){ ### for sites or repeats
  ...
  for (model in c("refLV","refVAR","randomLV","randomVAR"))
    ...
    alpha = seq(from = 0.01,to = 0.2, by = 0.02)
    nedges = seq(from = 16, to = 55, by  = 4)
    ...
    g.no<-getNetwork(res.noclust,nedges[threshold]) 
    ...
    pGC = pairwiseGC(abundance_mat,alpha[threshold],lag_order) ## alpha-level 0.05
...
pdf(file = "ROC_Simone_BIC_noClustering_woutIntraSpInteractions_changingThreshold.pdf",width=16,height = 8)
...
write.csv(scoresClassif,file="../results/changingThreshold/scoresClassif.csv")
write.csv(scores_pGC,file="../results/changingThreshold/scores_pGC.csv")