...
for (ksite in 1:nsites){ ### for sites or repeats
  ...
  for (model in c("refLV","refVAR","randomLV","randomVAR"))
    ...
    pGC = pairwiseGC(abundance_mat,alphaLevel,lag_order) ## alpha-level 0.05
...
pdf(file = paste("ROC_Simone_BIC_noClustering_woutIntraSpInteractions_BYcorrection_alpha",alphaLevel,".pdf",sep=""),width=16,height = 8)
