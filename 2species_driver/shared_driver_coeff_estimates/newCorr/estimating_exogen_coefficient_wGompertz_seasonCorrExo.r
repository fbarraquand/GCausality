...
for(noise in noise_seq){
	if(noise=="autocorrelatedANDseason"){
		correction_season_seq=c(0,1)
	}else{
		correction_season_seq=0
	}
...
if(correction==0){
	temp = species2_species1_temp[,name_exo]
	varcompet<-VAR(y=data.frame(cbind(species2_species1_temp[,name_x],species2_species1_temp[,name_y])), type="none",lag.max=20,ic="SC",exogen=data.frame(temp))
} else {
  temp = residuals(lm(species2_species1_temp[,name_exo] ~ seasonality[500:799]))
  varcompet<-VAR(y=data.frame(cbind(species2_species1_temp[,name_x],species2_species1_temp[,name_y])), type="none",lag.max=20,ic="SC",exogen=data.frame(cbind(temp,seasonality[500:799])))
}


summ_varcompet=summary(varcompet)
effect_temp1=summ_varcompet$varresult$X1$coefficients["temp",1]
effect_temp2=summ_varcompet$varresult$X2$coefficients["temp",1]
Pval_temp1=summ_varcompet$varresult$X1$coefficients["temp","Pr(>|t|)"]
Pval_temp2=summ_varcompet$varresult$X2$coefficients["temp","Pr(>|t|)"]
