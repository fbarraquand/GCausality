...
write(names_GC,file=paste("results/DataCompet_driver_inter_factorized_GC_otf_tmax800.csv",sep=""),sep=",",append=F,ncolumns=length(names_GC))
...
for(j in 1:3){
write(names_CCM,file=paste("results/DataCompet_driver_inter",end_id[j],"factorized_CCM_otf_tmax800.csv",sep=""),sep=",",append=F,ncolumns=length(names_CCM))
write(names_CCM,file=paste("results/DataCompet_driver_noInter",end_id[j],"factorized_CCM_otf_tmax800.csv",sep=""),sep=",",append=F,ncolumns=length(names_CCM))
}
...
#        y1_tmp=y1[501:800,kcond]-mean(y1[201:300,kcond])
        y1_tmp=c(scale(y1[501:800,kcond]))
        if(type_inter==1){
...
#                y_with[,2]=y_with[,2]-mean(y_with[,2])
                y_with[,1]=c(scale(y_with[,1]))
                y_with[,2]=c(scale(y_with[,2]))
               ...
               # y_without[,2]=y_without[,2]-mean(y_without[,2])
                y_without[,1]=c(scale(y_without[,1]))
                y_without[,2]=c(scale(y_without[,2]))
