...
species1_xmap_temp <- ccm(species2_species1_temp, E =  lag_order_inter_CCM_predictx[kcond,1], lib_column = "species1",
                      target_column = "temp", lib_size=libsizes, random_libs=FALSE) 
...
temp_xmap_species1 <- ccm(species2_species1_temp, E =  lag_order_inter_CCM_predicty[kcond,1], lib_column = "temp", target_column = "species1",
                      lib_sizes = libsizes, random_libs=FALSE)
...
RhoLMax_21_inter[kcond,1]=a_xmap_t$rho[a_xmap_t$lib_size==max(a_xmap_t$lib_size)] # 1 causes 2 if 2 xmap 1
RhoLMax_12_inter[kcond,1]=t_xmap_a$rho[t_xmap_a$lib_size==max(t_xmap_a$lib_size)] # 2 causes 1 if 1 xmap 2
...
for (i in 1:num_surr) {
  rho_surr1_twin$temp[i,1] <- ccm(cbind(species2_species1_temp$species1, surr_temp_twin[,i]), E = lag_order_inter_CCM_predictx[kcond,1], lib_column = 1, target_column = 2, lib_sizes = max(a_xmap_t$lib_size),random_libs = FALSE,num_samples=1)$rho #temp is the cause, so it is the target, and the "surrogated" time series, and the embedding is the one for species 1, ie lagx
  rho_surr1_twin$species1[i,1] <- ccm(cbind(surr_species1_twin[,i], species2_species1_temp$temp), E = lag_order_inter_CCM_predicty[kcond,1] , lib_column = 2, target_column = 1, lib_sizes = max(t_xmap_a$lib_size),num_samples=1,random_libs = FALSE)$rho #sp1 is the cause, so it is the target and the "surrogated" TS, and the embedding is the one for temperature, ie lagy
}

Pval_21_inter_CCM_surr[kcond,1]=sum(RhoLMax_21_inter[kcond,1]<rho_surr1_twin$temp) /num_surr
Pval_12_inter_CCM_surr[kcond,1]=sum(RhoLMax_12_inter[kcond,1]<rho_surr1_twin$species1) /num_surr
...
species1_xmap_species2 <- ccm(species2_species1_temp, E = lag_order_inter_CCM_predictx[kcond,2], lib_column = "species1",
                      target_column = "species2", lib_sizes = libsizes, random_libs = FALSE)
...
a_xmap_t <- ccm_means(species1_xmap_species2) #1 xmap 2
t_xmap_a <- ccm_means(species2_xmap_species1) #2 xmap 1
...
RhoLMax_21_inter[kcond,2]=a_xmap_t$rho[a_xmap_t$lib_size==max(a_xmap_t$lib_size)] # 1 causes 2 if 2 xmap 1
RhoLMax_12_inter[kcond,2]=t_xmap_a$rho[t_xmap_a$lib_size==max(t_xmap_a$lib_size)] # 2 causes 1 if 1 xmap 2
...
rho_surr1_twin<- list(species1 =  matrix(NA,nrow=num_surr,ncol=1), species2 =  matrix(NA,nrow=num_surr,ncol=1))

for (i in 1:num_surr) {
  rho_surr1_twin$species1[i,1] <- ccm(cbind(species2_species1_temp$species2, surr_species1_twin[,i]), E =  lag_order_inter_CCM_predicty[kcond,2], lib_column = 1, target_column = 2, lib_sizes =max(t_xmap_a$lib_size),num_samples=1,random_libs = FALSE)$rho #species 1 the cause so it is the surrogated TS and we use the embedding for species2
  rho_surr1_twin$species2[i,1] <- ccm(cbind(surr_species2_twin[,i], species2_species1_temp$species1), E =  lag_order_inter_CCM_predictx[kcond,2], lib_column = 2, target_column = 1, lib_sizes = max(a_xmap_t$lib_size),random_libs = FALSE,num_samples=1)$rho #species2 is te cause so it's the target and surrogated TS, and we use the embedding for species 2
}

#a_xmap_t is sp1_xmap_sp2; which means sp2 is the cause 
Pval_21_inter_CCM_surr[kcond,2]=sum(RhoLMax_21_inter[kcond,2]<rho_surr1_twin$species2) /num_surr
Pval_12_inter_CCM_surr[kcond,2]=sum(RhoLMax_12_inter[kcond,2]<rho_surr1_twin$species1) /num_surr
...
species2_xmap_temp <- ccm(species2_species1_temp, E =  lag_order_inter_CCM_predictx[kcond,3], lib_column = "species2",
                      target_column = "temp", lib_sizes = seq(5, 100, by = 5), random_libs = FALSE,replace=F)
...
a_xmap_t <- ccm_means(species2_xmap_temp)
t_xmap_a <- ccm_means(temp_xmap_species2)
### Using the same method for producing P-values as Cobey and Baskerville PloS One 2016
rho1xmap2_Lmin_random <- a_xmap_t$rho[a_xmap_t$lib_size ==libsizes[1]]
rho1xmap2_Lmax_random <- a_xmap_t$rho[a_xmap_t$lib_size ==max(a_xmap_t$lib_size)]
# Fraction of samples for which rho(L_max)<rho(L_min)
...
RhoLMax_12_inter[kcond,3]=t_xmap_a$rho[t_xmap_a$lib_size==max(t_xmap_a$lib_size)] # 1 causes 2 if 2 xmap 1
RhoLMax_21_inter[kcond,3]=a_xmap_t$rho[a_xmap_t$lib_size==max(a_xmap_t$lib_size)] # 2 causes 1 if 1 xmap 2
...
surr_temp_twin <- make_surrogate_data(species2_species1_temp$temp, method = "seasonal", num_surr = num_surr,T_period=24)
surr_species2_twin <- make_surrogate_data(species2_species1_temp$species2, method = "seasonal", num_surr = num_surr,T_period=24)
rho_surr1_twin<- list(temp =  matrix(NA,nrow=num_surr,ncol=1), species2 =  matrix(NA,nrow=num_surr,ncol=1))

for (i in 1:num_surr) {
  rho_surr1_twin$temp[i,1] <- ccm(cbind(species2_species1_temp$species2, surr_temp_twin[,i]), E =  lag_order_inter_CCM_predictx[kcond,3], lib_column = 1, target_column = 2, lib_sizes = max(a_xmap_t$lib_size),random_libs = FALSE,num_samples=1)$rho
  rho_surr1_twin$species2[i,1] <- ccm(cbind(surr_species2_twin[,i], species2_species1_temp$temp), E =  lag_order_inter_CCM_predicty[kcond,3], lib_column = 2, target_column = 1, lib_sizes = max(t_xmap_a$lib_size),random_libs = FALSE,num_samples=1)$rho
}

Pval_21_inter_CCM_surr[kcond,3]=sum(RhoLMax_21_inter[kcond,3]<rho_surr1_twin$temp) /num_surr
Pval_12_inter_CCM_surr[kcond,3]=sum(RhoLMax_12_inter[kcond,3]<rho_surr1_twin$species2) /num_surr
