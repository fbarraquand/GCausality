...
### Model selection is done by AIC                        ####
##############################################################
...
for (ksite in 1:nsites){ ### for sites or repeats
  ...
  for (model in c("randomLV","randomVAR"))
    ...
    g.no<-getNetwork(res.noclust,selection = "AIC") 
...
pdf(file = "../figures/ROC_Simone_AIC_noClustering_woutIntraSpInteractions_woutBHcorrection.pdf",width=16,height = 8)
...
write.csv(scoresClassif,file="../results/withoutIntraSp/noClustering/scoresClassif_AIC.csv")
write.csv(scores_pGC,file="../results/withoutIntraSp/noClustering/scores_pGC_AIC.csv")
